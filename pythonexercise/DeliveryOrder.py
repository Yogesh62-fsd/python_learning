...
class SaleOrders:
    ...
    def __init__(self):
        self.product_master = {
            'PRD1': {'name': 'pen', 'product_unit_price': 8, 'product_cost_price': 10, 'product_type': 'Stockable'},
            'PRD2': {'name': 'pencil', 'product_unit_price': 8, 'product_cost_price': 10, 'product_type': 'Stockable'},
            'PRD3': {'name': 'coffee', 'product_unit_price': 25, 'product_cost_price': 30,
                     'product_type': 'consumable'}}

        self.product_stock = {'PRD1': 1190, 'PRD2': 1198, 'PRD3': 1120}
        ...
        self.customer_address = {
            'Cust1': {'address 1': 'jay residency', 'address 2': 'krishna medical', 'city': 'rajkot',
                      'state': 'gujarat', 'country': 'india', 'zipcode': 857348},
            'Cust2': {'address 1': 'cws', 'address 2': 'jayant', 'city': 'singrauli', 'state': 'mp', 'country': 'india',
                      'zipcode': 853894}}

        self.sale_order = {'SO1': {'customer': 'Cust1',
                                   'order_lines': [
                                       {'product_sku': 'PRD1', 'unit_price': 8, 'quantity': 87, 'subtotal': 696,
                                        'state': 'cancel'},
                                       {'product_sku': 'PRD2', 'unit_price': 8, 'quantity': 90, 'subtotal': 720,
                                        'state': 'cancel'},
                                       {'product_sku': 'PRD3', 'unit_price': 25, 'quantity': 234, 'subtotal': 5850,
                                        'state': 'cancel'}],
                                   'order_date': '21:1:2023', 'state': 'cancel', 'order_total_amount': 7266},
                           'SO2': {'customer': 'Cust2',
                                   'order_lines': [
                                       {'product_sku': 'PRD1', 'unit_price': 8, 'quantity': 900, 'subtotal': 7200,
                                        'state': 'draft'},
                                       {'product_sku': 'PRD2', 'unit_price': 8, 'quantity': 890, 'subtotal': 7120,
                                        'state': 'draft'},
                                       {'product_sku': 'PRD3', 'unit_price': 25, 'quantity': 799, 'subtotal': 19975,
                                        'state': 'draft'}],
                                   'order_date': '21:1:2023', 'state': 'draft', 'order_total_amount': 34295}}
        self.delivery_order = {}
    ...
    def prepare_product(self):  # this method will take the input from user and generate product dictionary and return it
    ...
    def prepare_order(self):
        ...
        while True:
            ...
            customer_id = input(
                'please enter the Customer id for which you want to take the order from the above list:')
            print(customer_id)
            ...
            if orderid in self.sale_order.keys():
                ...
                if order_state in ('draft', 'done', 'confirm'):
                    if order_state == 'draft':
                        ...
                        print('Warning the Order Can not Draft Once Done !!!!!!!!!!!!')
                    pass
                else:
            ...
            if orderid in self.sale_order.keys():
                ...
                for statusorder in self.sale_order.get(orderid).get('order_lines'):
                    ...
                    if order_status in ('confirm', 'done', 'cancel'):
                        ...
                        print('Product  State has been changed successfully !!!!!!!!!!!!!!!!! ')
                self.create_delivery_order(orderid, self.sale_order.get(orderid))
    ...
    def set_done_order(self, delivery_orderno):
        while True:
            print('Deliver order are as follows:')
            # for order_id in delivery_orderno:
            print(self.delivery_order.get(delivery_orderno).get('salesorder'))
            orderid = self.delivery_order.get(delivery_orderno).get('salesorder')
            if orderid in self.sale_order.keys():
                order_state = self.sale_order.get(orderid).get('state')
                if order_state in ('cancel', 'draft'):
                    ...
                    print('Order State has been changed successfully  after the delivery validation !!!!!!!!!!!!!!!!!')
                    print('Initally stock were',self.product_stock)
                    for product in self.delivery_order.get(delivery_orderno).get('stock_moves'):
                        print(product.get('product_sku'), product.get('quantity'))
                        for product_id in self.product_stock:
                            if product_id==product.get('product_sku'):
                                self.product_stock[product_id]=self.product_stock[product_id]-product['quantity']
    ...
    def set_cancel_order(self):
        while True:
            ...
            if orderid in self.sale_order.keys():
                order_state = self.sale_order.get(orderid).get('state')
                if order_state in ('confirm', 'done','draft'):
                    if order_state == 'done':
                        print('Order is Done it cannot be Cancelled !!')
                    elif order_state=='draft':
                        self.sale_order.get(orderid)['state'] = 'cancel'
                        for order_line in self.sale_order.get(orderid).get('order_lines'):
                            order_line['state'] = 'cancel'
                    else:
                        allow_cancel=True
                        for delivery_id in self.delivery_order:
                             order_delivery_id=self.delivery_order.get(delivery_id).get('salesorder')
                             if order_delivery_id==orderid:
                                 if self.delivery_order.get(delivery_id).get('state')  in ('draft'):
                                     allow_cancel=False
                                     print('Warning the delivery order exists for this sale order it cannot be cancelled')
                        if allow_cancel:
                            self.sale_order.get(orderid)['state']='cancel'
                            for order_line in self.sale_order.get(orderid).get('order_lines'):
                                order_line['state']='cancel'
    ...
    def create_delivery_order(self, order_id, sale_order_details):

        print('In Delivery Order details are------------', order_id, sale_order_details)

        def generate_delivery_id(count):
            delivery_order_id = 'DO' + str(int(count))
            count += 1
            return delivery_order_id
        delivery_id = generate_delivery_id(self.countdelivery_id)
        self.countdelivery_id += 1

        self.delivery_order.update({delivery_id: {'salesorder': order_id, 'customer_id': sale_order_details.get('customer'),
                           'stock_moves': [{'product_sku': order_line.get('product_sku'), 'quantity': order_line.get('quantity'), 'state': 'draft'} for order_line in self.sale_order.get(order_id).get('order_lines') ],
                           'state': 'draft'}})
    ...
    def validate_delivery_order(self):
        ...
        print(self.delivery_order)
        while True:
            print('Delivery Order No as follows :')
            for deliveryno in self.delivery_order:
                print(deliveryno)
            deliveryno = input('Please Enter the delivery no for which you want to  validate delivery order:')
            print(deliveryno)
            if deliveryno in self.delivery_order.keys():

                if  self.delivery_order.get(deliveryno)['state'] in ('cancel'):
                    if self.delivery_order.get(deliveryno).get('state')=='cancel':
                        print('Delivery Order is cancel it cannot be done !!!!!!!')
                else:

                        self.delivery_order.get(deliveryno)['state'] = 'done'
                        self.set_done_order(deliveryno) # calling it to change the state of saleOrder
                        for status in self.delivery_order.get(deliveryno).get('stock_moves'):
                            status['state'] = 'done'
                        print('Delivery order validated !!!!!!!!!!!!!!!')
                print('AFter the validation the Delivery order is ',self.delivery_order)
                print('AFter the validation the sale order is ',self.sale_order)
                break
            else:
                print('Warning Please Enter the valid Order NO')
                while True:
    ...
    def cancel_delivery_order(self):  # this method is
        while True:
            print('Delivery Order No as follows :')
            for deliveryno in self.delivery_order:
                print(deliveryno)
            deliveryno = input('Please Enter the delivery no for which you want to  Cancel delivery order:')
            print(deliveryno)
            if deliveryno in self.delivery_order.keys():

                if   self.delivery_order.get(deliveryno)['state']=='done':
                    for status in self.delivery_order.get(deliveryno).get('stock_moves'):
                        if status['state']=='done':
                             print('Warning Delivery order is done it cannot be cancelled !!!!!!')
                else:
                    self.delivery_order.get(deliveryno)['state'] = 'cancel'
                    for status in self.delivery_order.get(deliveryno).get('stock_moves'):
                        status['state'] = 'cancel'
                    print('Delivery order cancelled !!!!!!!!!!!!!!!')
                print('AFter the cancel the Delivery order is ', self.delivery_order)
                print('AFter the cancel the sale order is ', self.sale_order)
                break
            else:
                print('Warning Please Enter the valid Delivery Order NO')
                while True:
                    print(" 1: Do you show details again ")
                    print("2: Exit")
                    choice = eval(input('select one of option given above:'))
...
while (True):
    ...
    if choice == 1:
        ...
        saleorderobj1.validate_delivery_order()
    elif choice == 9:
        saleorderobj1.cancel_delivery_order()
    elif choice == 10:
